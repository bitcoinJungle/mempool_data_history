...
WHERE a.block_timestamp IS NULL 
  ...
  AND (
    (SELECT MAX(block_height) FROM ${project_id}.${dataset_id}.${bloclevel_table} WHERE aggregated_by = "extended_blocklevel_update") IS NULL
    OR a.block_height >= (SELECT MAX(block_height) FROM ${project_id}.${dataset_id}.${bloclevel_table} WHERE aggregated_by = "extended_blocklevel_update")
  )
  AND a.txid = t.hash
