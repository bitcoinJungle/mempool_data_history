...
SET
  ...
  aggregated_by = "standard_blocklevel_update",
...
WHERE a.block_timestamp IS NULL 
  ...
  AND DATE(a.last_seen_timestamp) = DATE_SUB(CURRENT_DATE(), INTERVAL 1 DAY)
  AND (
    (SELECT MAX(block_height) FROM ${project_id}.${dataset_id}.${bloclevel_table} WHERE aggregated_by = "standard_blocklevel_update") IS NULL
    OR a.block_height >= (SELECT MAX(block_height) FROM ${project_id}.${dataset_id}.${bloclevel_table} WHERE aggregated_by = "standard_blocklevel_update")
  )
  AND a.txid = t.hash
                  --  REPLACE THAT BY A FILTER ON deduplicated_at NOT NULL BUT aggregated_at NULL ? INSTEAD FOR 'RETRIEVED MODE IN CASE OF ISSUE'
  AND t.block_timestamp BETWEEN TIMESTAMP_SUB(a.last_seen_timestamp, INTERVAL 20 MINUTES) AND TIMESTAMP_ADD(a.last_seen_timestamp, INTERVAL 1 HOUR);
